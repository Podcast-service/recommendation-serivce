...
class PodcastContentEventConsumerTest {

    private final PodcastContentEventService service = mock(PodcastContentEventService.class);
    private final SimpleMeterRegistry meterRegistry = new SimpleMeterRegistry();
    private final PodcastContentEventConsumer consumer = new PodcastContentEventConsumer(service, meterRegistry);

    @Test
    void ignoredEventIsAcknowledged() {
        ConsumerRecord<String, String> record = record("unknown-event-json");
        Acknowledgment acknowledgment = mock(Acknowledgment.class);
        when(service.handle(record.value())).thenReturn(ContentEventHandlingResult.IGNORED);
    ...
    private ConsumerRecord<String, String> record(String value) {
        return new ConsumerRecord<>("podcast.content.events.v1", 0, 10L, "key", value);
    }
}
